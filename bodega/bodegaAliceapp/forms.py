...
class FormDistribuidor(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'ciudad': forms.TextInput(
                attrs={'class': 'form-control', 'placeholder': 'Ciudad', 'required': 'required'}
            ),
    ...
    def clean_email(self):
        ...
        queryset = Distribuidor.objects.filter(email=email)
        if self.instance.pk:
    ...
    def clean(self):
        ...
        fecha_recepcion = cleaned_data.get('fechaRecepcion')
        if fecha_despacho and fecha_recepcion and fecha_despacho > fecha_recepcion:
            self.add_error(
                'fechaRecepcion',
                'La fecha de recepción debe ser igual o posterior a la fecha de despacho.',
            )
        return cleaned_data
    ...
    class Meta:
        ...
        widgets = {
            ...
            'precioUnitario': forms.NumberInput(
                attrs={'class': 'form-control', 'placeholder': 'Precio Unitario', 'required': 'required'}
            ),
            'iva': forms.NumberInput(attrs={'class': 'form-control', 'placeholder': 'IVA (%)', 'required': 'required'}),
            'descuentoTotal': forms.NumberInput(
                attrs={'class': 'form-control', 'placeholder': 'Descuento Total', 'required': 'required'}
            ),
        }
